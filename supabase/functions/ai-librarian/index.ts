...
const allowedOrigins = (Deno.env.get("CORS_ALLOWED_ORIGINS") || "http://localhost:5501,http://127.0.0.1:5501").split(",").map((origin) => origin.trim()).filter(Boolean);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!serviceRoleKey || !supabaseUrl) {
      return new Response(JSON.stringify({ answer: "AI Librarian config missing.", context: {} }), { status: 200, headers: corsHeaders(req) });
    }
    ...
    if (!geminiKey) {
      return new Response(JSON.stringify({ answer: fallbackAnswer(question, context), context, warning: "Missing GEMINI_API_KEY." }), { status: 200, headers: corsHeaders(req) });
    }
...
`;
  ...
  `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${geminiKey}`;
      ...
      if (!response.ok) {
        ...
        return new Response(JSON.stringify({ answer: fallbackAnswer(question, context), context, warning: `API error: ${errText}` }), { status: 200, headers: corsHeaders(req) });
      }
      ...
      return new Response(JSON.stringify({ answer: answer || fallbackAnswer(question, context), context }), { status: 200, headers: corsHeaders(req) });

    } catch (e: any) {
      console.error("Gemini API connection exception:", e);
      return new Response(JSON.stringify({ answer: fallbackAnswer(question, context), context, warning: e.message }), { status: 200, headers: corsHeaders(req) });
    }
  } catch (globalError: any) {
    return new Response(JSON.stringify({ answer: "System error.", context: {} }), { status: 500, headers: corsHeaders(req) });
  }
});
