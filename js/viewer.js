...
const refs = {
  ...
  researchIntelligencePanel: document.getElementById('researchIntelligencePanel'),
  researchIntelligenceContent: document.getElementById('researchIntelligenceContent'),
...
function updateStatus(pageIndex = 0) {
  if (refs.zoomStatus) refs.zoomStatus.textContent = `${Math.round(currentZoom * 100)}%`;
...
function hydrateSidebar(publication) {
  ...
  if (refs.researchIntelligencePanel && refs.researchIntelligenceContent) {
    const items = [
      ['Why it matters', publication.research_significance],
      ['Key findings', publication.key_findings],
      ['Methodology', publication.methodology],
      ['Data / resources', publication.data_resources],
      ['Research gaps', publication.research_gaps],
      ['Implications', publication.research_implications],
      ['Context', publication.context_geography],
      ['Limitations', publication.limitations],
      ['Recommended for', publication.recommended_for]
    ].filter(([,v]) => v);
    refs.researchIntelligenceContent.innerHTML = items.length
      ? items.map(([label,value]) => `<article><strong>${escapeHtml(label)}</strong><p>${escapeHtml(value)}</p></article>`).join('')
      : '<p class="muted">This publication has not yet been enriched with the extended research intelligence profile.</p>';
    refs.researchIntelligencePanel.classList.toggle('hidden', !items.length);
  }
...
function buildFlipDimensions() {
  ...
  const zoom = Math.max(0.7, Math.min(1.75, currentZoom));

  // Build the normal fitted page size first, then scale the actual PageFlip
  // canvas. This makes the zoom controls a real visual zoom rather than only
  // increasing the PDF rendering resolution. The shell is scrollable, so the
  // reader can inspect the page all the way up to 175%.
  let width;
  let height;

  if (isPortrait) {
    width = Math.min(shellWidth, Math.floor(shellHeight / pageAspect));
    height = Math.max(360, Math.floor(width * pageAspect));
  } else if (fitMode === 'width') {
    width = Math.floor(shellWidth / 2);
    height = Math.max(430, Math.min(shellHeight, Math.floor(width * pageAspect)));
  } else {
    height = Math.floor(shellHeight * 0.96);
  ...
  return {
    width: Math.max(260, Math.min(1800, Math.floor(width * zoom))),
    height: Math.max(360, Math.min(2200, Math.floor(height * zoom)))
  };
}
...
async function renderFlipbook(pdfUrl, forceReloadPdf = false) {
  ...
  const renderScale = Math.max(1.2, Math.min(4, (dims.width / 595) * 1.85));
...
refs.zoomIn?.addEventListener('click', async () => {
  currentZoom = Math.min(1.75, Math.round((currentZoom + 0.15) * 100) / 100);
  await rerender();
});

refs.zoomOut?.addEventListener('click', async () => {
  currentZoom = Math.max(0.7, Math.round((currentZoom - 0.15) * 100) / 100);
  await rerender();
});
