...
async function loadPublicationForEdit(id) {
  ...
  const cols = ['id','title','authors','type','year','abstract','research_areas','isbn','doi','citation','publication_format','price','is_paid','preview_page_limit','cover_url','pdf_url','pdf_path','ebook_url','ebook_entry','ebook_path','preview_url','research_significance','key_findings','methodology','data_resources','research_gaps','research_implications','context_geography','keywords','limitations','recommended_for'];
  ...
  document.getElementById('researchSignificance').value = data.research_significance || '';
  document.getElementById('keyFindings').value = data.key_findings || '';
  document.getElementById('methodology').value = data.methodology || '';
  document.getElementById('dataResources').value = data.data_resources || '';
  document.getElementById('researchGap').value = data.research_gaps || '';
  document.getElementById('researchImplications').value = data.research_implications || '';
  document.getElementById('contextGeography').value = data.context_geography || '';
  document.getElementById('keywords').value = Array.isArray(data.keywords) ? data.keywords.join(', ') : (data.keywords || '');
  document.getElementById('researchLimitations').value = data.limitations || '';
  document.getElementById('recommendedFor').value = data.recommended_for || '';
...
async function handleEditSubmit(e) {
  ...
  try {
    ...
    const research_significance = document.getElementById('researchSignificance').value.trim() || null;
    const key_findings = document.getElementById('keyFindings').value.trim() || null;
    const methodology = document.getElementById('methodology').value.trim() || null;
    const data_resources = document.getElementById('dataResources').value.trim() || null;
    const research_gaps = document.getElementById('researchGap').value.trim() || null;
    const research_implications = document.getElementById('researchImplications').value.trim() || null;
    const context_geography = document.getElementById('contextGeography').value.trim() || null;
    const keywords = normalizeAreas(document.getElementById('keywords').value || '');
    const limitations = document.getElementById('researchLimitations').value.trim() || null;
    const recommended_for = document.getElementById('recommendedFor').value.trim() || null;
    ...
    const payload = {
      ...
      year,
      abstract,
      research_significance, key_findings, methodology, data_resources, research_gaps, research_implications, context_geography, keywords, limitations, recommended_for,
      research_areas,
      isbn,
      doi,
      citation,
